...
export default function App() {
  ...
  const current = useMemo(
    () => data.sessions.find((session) => !session.endTime) ?? null,
    [data.sessions]
  )

  const updateSessions = (sessions: SleepSession[]) => {
  ...
  const endNow = () => {
    ...
    updateSessions(
      data.sessions.map((session) =>
        session.id === current.id ? { ...session, endTime, updatedAt: endTime } : session
      )
    )
  }
  ...
  const saveEditor = (session: SleepSession) => {
    if (editor === 'new') {
      updateSessions([session, ...data.sessions])
    } else {
      updateSessions(data.sessions.map((item) => (item.id === session.id ? session : item)))
    }
    setEditor(null)
  }
  ...
  return (
    <div className="app-shell">
      <main className="app-main">
        ...
        {page === 'settings' && (
          <SettingsPage data={data} setData={setData} onBack={() => setPage('today')} />
        )}
      </main>
...
function TodayPage({ data, now, current, onStart, onEnd, onOpenEditor, onSettings }: TodayPageProps) {
  const todays = todaySessions(data.sessions).sort(
  ...
  return (
    <section className="screen today-screen">
      <header className="compact-header">
        <div>
          ...
          <div className="daily-summary">
            Ma eddig <strong>{formatDuration(total)}</strong> alvás
          </div>
        </div>
        <button className="icon-button" aria-label="Beállítások" onClick={onSettings}>
          ⚙︎
        </button>
      </header>
      ...
      <button className="primary-action" onClick={current ? onEnd : onStart}>
        {current ? '☀  Felébredt' : '☾  Elaludt'}
      </button>
      <button className="text-action" onClick={() => onOpenEditor(current ?? 'new')}>
        Részletek
      </button>

      <div className="section-head">
        <h2>Mai alvások</h2>
        <span>{todays.length} alkalom</span>
      </div>
...
function SleepRow({ session, now, onClick }: { session: SleepSession; now: number; onClick?: () => void }) {
  return (
    <button className="sleep-row" onClick={onClick}>
      ...
      <span className="sleep-row-time">
        {formatTime(session.startTime)} – {session.endTime ? formatTime(session.endTime) : 'most'}
      </span>
...
function HistoryPage({ sessions, onEdit, onNew }: { sessions: SleepSession[]; onEdit: (s: SleepSession) => void; onNew: () => void }) {
  const grouped = useMemo(() => {
    ...
    sortedSessions.forEach((session) => {
      ...
      map.set(key, currentItems.concat(session))
    })
  ...
  return (
    <section className="screen">
      <header className="page-header">
        <h1>Előzmények</h1>
        <button className="add-button" onClick={onNew}>＋</button>
      </header>
...
function StatsPage({ sessions, now }: { sessions: SleepSession[]; now: number }) {
  ...
  const chart = useMemo(
    () =>
      Array.from({ length: days }, (_, i) => {
        ...
        return {
          label: new Intl.DateTimeFormat('hu-HU', { day: 'numeric' }).format(d),
  ...
  const recent = sessions.filter(
    (session) => new Date(session.startTime).getTime() >= now - days * 86400000
  )
    ...
    <section className="screen">
        ...
        <div className="bar-chart">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chart}>
              ...
              <Tooltip
                contentStyle={{ background: '#0d1a2b', border: '1px solid #1c3352', borderRadius: 12 }}
                formatter={(value) => [`${value} ó`, 'Alvás']}
              />
              <Bar dataKey="hours" fill="#55a7ff" radius={[8, 8, 4, 4]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="chart-card radial-card">
        ...
        <div className="radial">
          <div className="radial-inner"><span>☾</span><strong>24h</strong><span>☀</span></div>
        </div>
...
function SettingsPage({ data, setData, onBack }: { data: AppData; setData: (data: AppData) => void; onBack: () => void }) {
  const changeName = (e: ChangeEvent<HTMLInputElement>) => {
    setData({ ...data, settings: { ...data.settings, childName: e.target.value } })
  }
  ...
  const handleImport = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!window.confirm(`${next.sessions.length} alvásbejegyzést találtam. Felülírjam a jelenlegi adatokat?`)) {
        return
      }
      exportData(data)
      setData(next)
    } catch (err) {
      window.alert(err instanceof Error ? err.message : 'Importálási hiba.')
    }
  ...
  const clear = () => {
    ...
    setData({
      ...defaultData,
      settings: { ...defaultData.settings, childName: data.settings.childName || 'Botond' }
    })
  }
  ...
  return (
    <section className="screen settings-screen">
      ...
      <div className="settings-card">
        <label>
          Baba neve
          <input value={data.settings.childName} onChange={changeName} placeholder="Botond" />
        </label>
      </div>
...
function SleepEditor({ session, currentExists, onClose, onSave, onDelete }: {
  ...
  const submit = () => {
    ...
    if (
      new Date(startIso).getTime() > Date.now() + 60000 ||
      (endIso && new Date(endIso).getTime() > Date.now() + 60000)
    ) {
      window.alert('Jövőbeli időpont nem menthető.')
      return
    }

    if (endIso && new Date(endIso) <= new Date(startIso)) {
      window.alert('A felébredésnek később kell lennie, mint az elalvásnak.')
      return
    }

    if (stillSleeping && currentExists && !session) {
      window.alert('Már van egy futó alvás.')
      return
    }

    const nowIso = new Date().toISOString()
    if (session) {
      onSave({ ...session, startTime: startIso, endTime: endIso, note, updatedAt: nowIso })
    } else {
      onSave({ ...createSession(startIso, endIso), note })
    }
  }
    ...
    <div className="editor-overlay">
      <div className="editor-screen">
        <header className="editor-header">
          ...
          <h1>{session ? 'Részletek' : 'Alvás rögzítése'}</h1>
          <span />
        </header>
        ...
        <div className="editor-body">
          ...
          <div className={`picker-block ${stillSleeping ? 'disabled' : ''}`}>
            <label>☀ Felébredt</label>
            <input
              className="datetime-picker"
              type="datetime-local"
              value={end}
              max={toLocalInput()}
              disabled={stillSleeping}
              onChange={(e) => setEnd(e.target.value)}
            />
          </div>

          <label className="toggle-row">
            <span>🛏 Még alszik</span>
            <input
              type="checkbox"
