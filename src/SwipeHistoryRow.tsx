...
export default function SwipeHistoryRow({ session, now, onEdit, onDelete }: { session: SleepSession; now: number; onEdit: () => void; onDelete: () => void }) {
  ...
  return <div className="swipe-history-row" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} onTouchCancel={handleTouchEnd}>
    <div className="swipe-history-actions">
      <button className="swipe-edit" type="button" onClick={() => { setOffset(0); onEdit() }}><EditIcon /><span>Szerkesztés</span></button>
      <button className="swipe-delete" type="button" onClick={() => { setOffset(0); onDelete() }}><TrashIcon /><span>Törlés</span></button>
    </div>
