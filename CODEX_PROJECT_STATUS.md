...
**Aktuális HEAD:** `1547fa7` a `feat/child-profile-v4` ágon; az A24 PWA-frissítési és offline fiókindítási javításokat a tulajdonos commitolta. Az A15 helyi javításai elkészültek, még nincsenek commitolva: külön naplóválasztás és mentés csatlakozáskor, gyermekprofil-konfliktusok, párhuzamos törlés/indítás, atomi letöltés és importkorrekciók. 31 fájl / 371 teszt, mindkét typecheck, build és elkülönített mobilméretű Edge UI-próba sikeres. Staging/telefonos A15-elfogadás még nincs; a családmegszüntetés korábbi kézi visszatesztje is nyitott. Távoli családi adatot nem módosítottunk. Részletek: [FAMILY_JOIN_CHECKPOINT_2026-09-26.md](FAMILY_JOIN_CHECKPOINT_2026-09-26.md), [PWA_UPDATE_CHECKPOINT_2026-09-26.md](PWA_UPDATE_CHECKPOINT_2026-09-26.md), [FAMILY_DISSOLUTION_CHECKPOINT_2026-09-23.md](FAMILY_DISSOLUTION_CHECKPOINT_2026-09-23.md).

**Következő önálló feladat — GPT-6 Astra · erős:** A16 — offline jogosultság és párhuzamos bejelentkezés-frissítés ellenőrzése, automatikus próbákkal. A tulajdonos jelzi, amikor újra tud kézzel tesztelni. Addig a staging/telefonos kapuk nyitva maradnak; commit/push a tulajdonos feladata.
...
- **Elkülönített törlési próba előfeltétele:** a családmegszüntetési visszaút helyben elkészült, de stagingen még nincs elfogadva. Az eredeti Edge-admin maradjon az Opo családban; csak a nem admin tesztfiókot visszük egy kis külön családba az új build után, két elkülönített böngészőkörnyezetben. Az Opo naplója és a feleség tagsága megmarad. Az adatbázismentést a [STAGING_D1_RECOVERY_2026-09-23.md](STAGING_D1_RECOVERY_2026-09-23.md), a lépéssort az új családmegszüntetési checkpoint rögzíti.
...
**2026-09-26, A24 PWA helyi részeredmény — [GPT-6 Astra · high]:** az app nem aktivál frissítést és nem tölt újra automatikusan nyitott szerkesztés közben. Az új SW megvárja a régi appablakok bezárását. Az offline/hálózati hibás fiókvisszaállítás többé nem vált vendégnaplóra; meghagyja az aktív fiók naplóját és outboxát, és engedi a későbbi újrapróbálást. Igazoltan érvénytelen session esetén továbbra is megmarad a kijelentkezési átmenet. **30 fájl / 338 teszt**, mindkét typecheck és az internal build sikeres. Elkülönített Edge-ben `/` és `/botond-sleep-tracker/` alatt sikeres a hibás letöltés, kétlapos várakozás, szerkesztő megőrzése, fiókos offline indulás és azonos adatformátumú tesztartifact visszaállítása. Aktív alvás, négy függő művelet és másik fiók mentése megmaradt. A próbák jelenlegi forrásból készült A/B artifactokat használnak; történeti verzió és telepített telefonos PWA elfogadását nem helyettesítik. Részletes bizonyíték, ismételhető parancs és rollback-határok: [PWA_UPDATE_CHECKPOINT_2026-09-26.md](PWA_UPDATE_CHECKPOINT_2026-09-26.md). A24, A05 és A16 még nyitott.

**Következő önálló feladat — [GPT-6 Astra · high]:** A15 — nem üres helyi napló családhoz csatlakozásának, gyermekütközéseinek és letöltési határeseteinek kódellenőrzése, célzott automatikus próbákkal. Kézi teszt nélkül kezdhető. A tulajdonos jelzéséig telefonos próbát nem kérünk; commit/push továbbra is a tulajdonos feladata.

**2026-09-26, A24 staging commit-kapu — [GPT-6 Astra · high]:** a `deploy:staging` a checkout teljes SHA-ját fordításkor beépíti a Workerbe, ellenőrzi a Cloudflare `WORKERS_CI_COMMIT_SHA` egyezését, és nem enged azonosított deployt nem commitolt fájlokból. A helyi dry-run megengedett, módosított checkouton `local` jelzést kap. A Worker health-válasza és minden válaszfejléce jelzi a buildet. A staging smoke kötelezően a GitHub esemény SHA-ját várja, legfeljebb 8 percig csak health-et kérdez, majd minden auth-próbán és a záró health-en is ellenőrzi az egyezést. Régi/hiányzó build, timeout és teszt közbeni verzióváltás hibát ad. A CI a Worker helyi dry-run csomagolását is futtatja. Ellenőrzés: **28 fájl / 324 teszt**, frontend és Worker typecheck, helyi Wrangler-csomagolás sikeres; a csomagolt SHA futásidejű felülírással sem változott. A használt helyi Wrangler 4.131.1; a lockfile szerinti tiszta CI-futás külön ellenőrizendő. Távoli deploy és adatváltoztatás nem történt. A commit/push a tulajdonos feladata; utána a tényleges Cloudflare-build és az ugyanarra a SHA-ra sikeres smoke eredménye még szükséges. A24 és A04 nincs lezárva.

**Staging commit-kapu utóellenőrzés, 2026-09-26 — [GPT-6 Sol · medium]:** a tulajdonos `242770e` commit/pushja után a staging `/health` és minden smoke-válasz a teljes `242770e5740f4cd9e9d6a642b27241d1c7422140` SHA-t adta; a helyben indított smoke minden ponton sikeres. GitHub Actions futásrekordot a connector nem adott, így annak saját futása továbbra sem igazolt. A fenti helyi PWA-részeredmény az ezt követő munkamenet eredménye.

**2026-09-26, további A24 helyi részeredmény — [GPT-6 Sol · medium]:** a GitHub Pages deploy munkamenete csak kézi indítással, `main` ágon épít; a saját checkoutján frontend és Worker typechecket, teljes tesztcsomagot futtat a production build előtt. A build SHA-t beírja az artifactba. A jelenlegi A07 production konfigurációs kapu a publikálást továbbra is megállítja, amíg az éles környezet nincs kész. Az internal artifact build fiókos családi kapcsolói és staging sync URL-je igazodnak az élő belső tesztoldalhoz; a Worker typecheck is része a munkamenetnek. Az `index.html` ikonlinkjei a build alapútvonalát használják; a gyökér- és alkönyvtáras buildben is a helyes URL-t adták, az ikon ténylegesen 1024×1024 képpont. Mindkét helyi build sikeres. A GitHub-munkamenet tényleges futása és a PWA upgrade/rollback még nyitott.

**2026-09-26, felhasználói teszt nélküli A24 részeredmény — [GPT-6 Sol · medium]:** a frontend és a Worker függőségei rögzített lockfile-t kaptak; a CI, a belső preview és a Pages build `npm ci` telepítésre váltott. A két lockfile külön tiszta könyvtárban végzett `npm ci --dry-run` ellenőrzése sikeres. A tulajdonos `88a1654` commit/push után jelezte, hogy most nem tud kézzel tesztelni. A tényleges GitHub Actions futás külön ellenőrizendő. A24 többi kiadási kapuja nyitott; az A04 staging megszüntetési visszatesztjét ez nem helyettesíti.

**Aktuális megálló, 2026-09-25 — [GPT-6 Astra · high]:** A családmegszüntetés utáni téves hibaválasz helyben javítva: a D1 a családsorral együtt kaszkádolt tagsági törléseket is beleszámolja a meta.changes értékbe, ezért a korábbi pontosan 1-es ellenőrzés siker után is 409-et adott. A feltétel most pozitív módosításszámot fogad el, a nulla továbbra is ütközés. A tesztadapter D1-kompatibilis számlálást használ; a régi kód két meglévő Free/fizetős megszüntetési tesztben reprodukálta a hibát. Javítás után 27 fájl / 296 teszt és Worker typecheck sikeres. Következő: a staging Worker-verzió ellenőrzése, majd amikor a tulajdonos újra tud tesztelni, külön kis tesztcsalád megszüntetése és a hibamentes válasz kézi ellenőrzése. Staging elfogadás még nincs; A04 nyitott.

**[GPT-6 Sol · medium]** A külön tesztcsalád teljes kézi staging lánca sikeres: 1 gyermek/1 alvásos szerveres export, végleges családmegszüntetés, mindkét tesztböngésző családi kapcsolatának megszűnése, bejelentkezett fiók és helyi Tesztbaba / 1 alvás megőrzése, Chrome külön helyi törlése, majd tiszta visszacsatlakozás az Opo családba. A03 online-pending importtiltás is kézzel igazolt: Chrome online, `sessions` feltöltés blokkolva, 1 várakozó módosítás, a friss Opo-export importját az app a várt üzenettel megállította. A blokkolás feloldása után a tesztalvás mindkét böngészőben megjelent, majd az azonosított 21:15–21:18-as próbasor törlése után Edge és Chrome egyaránt Boti/1799-et mutat. Ez a kézi próbakör lezárva. A tulajdonos szerint a feleség saját eszközén is Opo / friss szinkron / 1799 alvás látható; a panelben nincs taglista. A04 Free utolsó admin külön `Solemi Free-admin próba` családjának megszüntetése, mentése és a helyi/fiókadatok megőrzése is kézi visszajelzéssel sikeres; az Edge Opo/1799 változatlan. A megszűnt család régi meghívójára a Chrome `A meghívókód nem található` üzenetet kapott, és családon kívül maradt. A kifejezett adminátadás és az eredeti szerepek helyreállítása az oda-vissza próbával elfogadva; a családmegszüntetés utáni téves hibaválasz helyi javításának staging elfogadása nyitott.
